...
class CustomerChurnSchema(BaseModel):
    ...
    #TODO: update and check validators
    # ---------------------------
    # Validators
    # ---------------------------
    @validator("credit_utilization_ratio", pre=True, always=True)
    def compute_credit_utilization(cls, v, values, **kwargs):
        """Robustly compute utilization ratio.

        - If credit_limit is None or <= 0 -> utilization is None (undefined).
        - If revolving_balance is None -> treat as 0.0.
        - Otherwise compute min(revolving_balance / credit_limit, 1.0).
        """
        credit_limit = values.get("credit_limit")
        revolving_balance = values.get("revolving_balance")
        if credit_limit is None or credit_limit <= 0:
            return None
        rev = 0.0 if revolving_balance is None else float(revolving_balance)
        try:
            return min(rev / float(credit_limit), 1.0)
        except Exception:
            return v

    @validator("transaction_count_12m")
    def check_transaction_counts(cls, v, values):
        count_3m = values.get("transaction_count_3m", 0)
        if v < count_3m:
            raise ValueError("12M transaction count cannot be less than 3M transaction count")
        return v

    @validator("transaction_volume_12m", always=True)
    def check_transaction_volumes(cls, v, values):
        """Ensure 12M volume >= 3M volume and zero-count consistency."""
        vol_3m = values.get("transaction_volume_3m", 0)
        cnt_3m = values.get("transaction_count_3m", 0)
        if v is not None and v < vol_3m:
            raise ValueError("transaction_volume_12m cannot be less than transaction_volume_3m")
        if cnt_3m == 0 and vol_3m and vol_3m > 0:
            raise ValueError("transaction_volume_3m > 0 while transaction_count_3m == 0")
        return v

    @validator("number_of_accounts")
    def check_account_breakdown(cls, v, values):
        """Ensure account breakdown does not exceed total number_of_accounts."""
        sum_breakdown = (
            values.get("num_standard_accounts", 0)
            + values.get("num_savings_accounts", 0)
            + values.get("num_student_accounts", 0)
            + values.get("num_business_accounts", 0)
        )
        if v < sum_breakdown:
            raise ValueError("number_of_accounts cannot be less than sum of account-type breakdowns")
        return v

    @validator("tenure_months")
    def tenure_matches_signup(cls, v, values):
        """If signup_date present, check tenure_months is consistent with snapshot_date."""
        signup = values.get("signup_date")
        snap = values.get("snapshot_date")
        if signup and snap:
            months = (snap.year - signup.year) * 12 + (snap.month - signup.month)
            if abs(months - v) > 2:
                # allow small rounding differences, but flag large mismatches
                raise ValueError("tenure_months inconsistent with signup_date and snapshot_date")
        return v

    @validator("churn", always=True)
    def churn_recency_consistency(cls, v, values):
        """If churn label is True, ensure recency indicates inactivity (business rule).

        This enforces a label definition of inactivity >= 90 days. Modify the
        threshold if your business defines churn differently.
        """
        days = values.get("days_since_last_activity")
        if v:
            if days is not None and days < 90:
                raise ValueError("churn=True but days_since_last_activity < 90 (label inconsistent)")
        return v
